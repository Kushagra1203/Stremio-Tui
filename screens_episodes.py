...
from widgets import SeriesSidebar, fmt_runtime
...
class SeriesDetailScreen(Screen):
    ...
    def on_mount(self):
        self.controller = SeriesController(self.app.client)
        self.fetch_data()
    ...
    async def fetch_data(self):
        ...
        self.meta = await self.controller.get_unified_metadata(self.imdb_id, self.show_title)
        ...
        # 3. Fetch Season Metadata (Smart Fetch: AniList OR TVMaze)
        self.query_one("#screen_title").update("Fetching Season Metadata...")
        
        # Pass COUNTRY here to enable the "Hard Check"
        self.season_meta_cache = await self.controller.fetch_all_season_details(
            self.imdb_id, 
            self.show_title, 
        ...
        if self.main_poster_url:
            await self.controller.get_image(self.main_poster_url)
        ...
        for s_num in keys:
            ...
            if s_key in self.season_meta_cache:
                ...
                if s_poster:
                    await self.controller.get_image(s_poster)
            count += 1
    ...
    async def load_image_to_sidebar(self, url):
        pil_img = await self.controller.get_image(url)
    ...
    async def lazy_load_season(self, season_num):
        ...
        # Check if we have an AniList rating for this season
        anilist_rating = None
        if season_num in self.season_meta_cache:
            anilist_rating = self.season_meta_cache[season_num].get('rating')

        # Try OMDb
        ratings_map = await self.controller.fetch_season_ratings(self.imdb_id, season_num)
        ...
        for ep in eps:
            ...
            # Priority 2: AniList Season Rating (Fill in the gaps!)
            elif anilist_rating:
    ...
    async def prefetch_images(self, season_num):
        ...
        for ep in target_eps:
            ...
            if url:
                await self.controller.get_image(url)
