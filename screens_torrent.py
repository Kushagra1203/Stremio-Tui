...
class StreamSelectScreen(Screen):
    ...
    # CSS: Full screen, no scrollbar
    CSS = """
    StreamSelectScreen {
        layer: above; /* Ensure it covers everything */
        layout: vertical;
    }
    ...
    async def fetch_streams(self):
        app = self.app
        # This triggers the debug prints we just used!
        ...
        for s in streams:
            ...
            if not filename: filename = get_magnet_name(s.get('url') or s.get('infoHash'))
